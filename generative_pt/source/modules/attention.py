...
class CopyAttention(nn.Module):
    ...
    def forward(self, query, memory, coverage, mask=None):
        # declare the size
        ...
        output_expand = query.expand(batch_size, input_size, hidden_size)

        # reshape to 2-dim
        output_expand = output_expand.reshape([-1, hidden_size])
        context = memory.reshape([-1, hidden_size])
        ...
        # update the coverage
        coverage = coverage.reshape(batch_size, -1, input_size) + attn
