...
class RNNDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        if self.goal_size is not None:
            ...
            self.out_input_size += self.goal_size
            self.cue_input_size += self.goal_size
    ...
    def decode(self, input, state, is_training=False):
        ...
        if state.goal is not None:
            rnn_input_list.append(state.goal)
            cue_input_list.append(state.goal)
            out_input_list.append(state.goal)
    ...
    def forward(self, inputs, state, is_training, epoch):
        ...
        if use_teacher_forcing:
            ...
            for i, num_valid in enumerate(num_valid_list):
                ...
                _, topi = log_softmax_output.data.topk(1)

                input_var = topi.squeeze(1)

                log_probs[:num_valid, i] = log_softmax_output[:num_valid]
